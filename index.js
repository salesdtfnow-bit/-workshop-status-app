...
async function getOrderQueuedCm(orderGid) {
  const data = await shopifyGraphQL(`
    query OrderQueued($id: ID!) {
      order(id: $id) {
        metafield(namespace: "workshop_status", key: "dtf_queued_cm") { value }
      }
    }
  `, { id: orderGid });
...
app.post('/api/webhooks/orders', async (req, res) => {
  ...
  try {
    if (topic === 'orders/paid') {
      // Compute meterage from line items
    ...
    // Return 200 anyway so Shopify doesn't retry on errors that won't recover
